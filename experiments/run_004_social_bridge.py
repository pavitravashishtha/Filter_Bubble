import sys
import os
# Allow running as both `python experiments/run_004_social_bridge.py` and `python -m experiments.run_004_social_bridge`
...
def run_full_research_simulation():
    config = SimulationConfig(
        ...
        intervention_type = "social_bridge",  # changed from diversity_injection
        intervention_strength = 0.2,
        new_agent_entry_rate = 1,
    )
    
    print("=== FILTER BUBBLE SIMULATION — RESEARCH RUN 004 ===")
    ...
    with open("experiments/run_004_summary.json", "w") as f:
        json.dump(results_summary, f, indent=2)
    print("Summary saved to experiments/run_004_summary.json")
